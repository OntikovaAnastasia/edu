...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (!isset($arParams['AGENTS_COUNT'])) {
            $arParams['AGENTS_COUNT'] = 10;
            }
        /*
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
        ...
        //$hlblock = HighloadBlockTable::getById($arHlblock)->fetch();   
        $entitys = HighloadBlockTable::compileEntity($arHlblock);
        $entity_data_class = $entitys->getDataClass();
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
            ...
            $enumList = \CUserFieldEnum::getList($fieldID);
            
            while($arEnum = $enumList->GetNext())
            {
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        while ($arAgent = $rsAgents->fetch()) {
           ...
           // echo '<pre>';print_r($arAgent);echo '</pre>'; 
           // echo '<pre>';print_r($arAgent['ID']);echo '</pre>'; 
            $key = $arAgent['UF_TYPE_ACTIVITY'];
     
      
       //$rsTypeAgents = $arTypeAgents->fetchAll();

        if (!empty($arAgent["UF_PHOTO"])) {
            $arFile = CFile::GetPath($arAgent["UF_PHOTO"]);
            }
            else{
                $arFile = "/local/components/mcart/agents.list/templates/.default/images/no-avatar.png";
            }
            $arAgent['UF_PHOTO'] =  $arFile;
            $arAgent['UF_TYPE_ACTIVITY'] =  $arTypeAgents["$key"];
            /**
    ...
    public function clickStarAction($agentID)
    {
        ...
        $arView = CUserOptions::GetOption("mcart_agent", "options_agents_star");
          ...
          if(isset($arView))
        {
            ...
            { 
                ...
                if(in_array($agentID , $arView))
                {
                    $keys = array_search($agentID, $arView);
                    unset($arView["$keys"]);
                }
                else{
                    array_push($arView, $agentID);
                }
                $value = $arView;
            } 
        }
